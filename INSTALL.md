...
- доступы к четырём репозиториям команды (системный анализ, фронтенд, бэкенд,
  автотесты): их рабочие копии вы клонируете сами — **плагин не клонирует**.
...
`repos/autotests` (таблица «Какая конфигурация нужна какой команде» — в
[README.md](README.md)).
...
2. **MCP-сервер Jenkins**, подключённый в вашем клиенте (Claude Code —
   ...
   Плагин его не устанавливает и не настраивает.
...
| Этап останавливается: в `repos.*.link` git-URL или путь наружу | В `settings.json` нужен путь ВНУТРИ рабочего репозитория (`repos/frontend`). git-URL и пути вроде `../frontend` guard-скрипты не принимают. Поправьте `settings.json` и закоммитьте — он общий для команды. |
