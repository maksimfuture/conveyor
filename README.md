...
│                         #   scope (рабочая область), validate-artifact
├── adapters/
...
- **Совет по скорости:** заведите в каждом из четырёх репозиториев файл
  ...
  вместо самостоятельного исследования репозитория — меньше чтений и
  быстрее на слабых моделях.
- **fast** — быстрый режим для медленных/слабых моделей, из env
  `CONVEYOR_FAST` (по умолчанию ВЫКЛЮЧЕН). Включает: ревью отключено
  (reviewRounds принудительно 0), валидация артефактов скриптом
  `validate-artifact.mjs` вместо повторного прогона агента, короче
  git-контекст и диффы, краткое заполнение шаблонов. Детали —
...
(артефакты `tasks/` и temp разрешены всегда; `scope.mjs clear` — снять, в том
числе при досрочной остановке этапа; устаревшая область игнорируется по TTL
8 ч и снимается на старте сессии).

| Этап | Запись в репо |
|---|---|
| create-feature | только анализ (systemsAnalysis) |
| implement-plan | только код FE/BE (по типу задачи) |
| implement-auto-test | только автотесты (autoTest) |
| остальные | никакие — только артефакты задачи |

Агенту передаются только пути его области; диффы чужих репозиториев готовит
команда через `git-ops.mjs` и передаёт текстом. Промпты агентов содержат
явный запрет открывать чужие репозитории.
