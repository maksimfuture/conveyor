...
4. Запусти qa-autotest-engineer: вход — specification.md (основа плана),
   ...
   `- [ ]`: файл + действие + `Кейсы: TC-N` + `Закрывает: REQ-N`) →
   «Тест-кейсы» → данные → «Что не автоматизируем». Галочки шагов на этом
   этапе не расставляются.
5. Валидация `validate-artifact --type autotest-plan` плюс покрытие критериев
   приёмки и сходимость «Итого» с разделами ниже; возврат агенту не только
   при `ok:false`, но и при непустом `placeholders`. Цикл ревью НЕ
   запускается.
