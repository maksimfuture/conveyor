...
- пиши только внутри: рабочего репозитория (`docs/specs/tasks/`, `intents/` и
  файлы конфигурации в корне), рабочих копий из области текущего этапа
  (`repos/*`), системного temp;
- в папке задачи (`docs/specs/tasks/<FE|BE>/<TASK-ID>/`) и папке намерения
  (`intents/<INTENT-ID>/`) — ТОЛЬКО артефакты (`*.md`, `meta.json`).
  ИСХОДНИКИ пиши исключительно в рабочую копию кодовой базы; файл кода в
  `docs/specs/tasks/` — ошибка (guard-writes её блокирует). Перед
...
- вызов guard-скрипта — это ВСЯ проверка (dry-run). НИКОГДА не создавай
  ...
  рабочего репозитория вне `docs/specs/tasks/` и `intents/`;
