...
- в папке задачи (`tasks/<FE|BE>/<TASK-ID>/`) — ТОЛЬКО артефакты
  (`*.md`, `meta.json`). ИСХОДНИКИ пиши исключительно в рабочую копию
  кодовой базы; файл кода в tasks/ — ошибка (guard-writes её блокирует).
  Перед `stages.<этап>.done = true` прогоняй
