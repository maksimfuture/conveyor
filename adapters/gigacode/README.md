...
`<CONVEYOR_ROOT>` в текстах команд — ПЛЕЙСХОЛДЕР, а не переменная окружения.
Каждая команда начинается с шага «Корень расширения»: он сам печатает
абсолютный путь установки (`~/.gigacode/extensions/conveyor`) и проверяет, что
там лежит ядро; дальше агент подставляет напечатанное буквально. Каталога нет —
этап останавливается и спрашивает путь у пользователя.
