---
description: Составляет план автотестов по спецификации задачи (две таблицы: тест-кейсы с шагами, ожидаемым результатом и покрытием да/нет в строке плюс автотесты с классом и тем, что именно реализовать; ID теста из Jira @TmsLink, колонка «Вид тестирования» НФ/Регресс, трассировка на REQ-N). Контекст — репозиторий автотестов (только чтение); кодовая база не открывается. Запускать после implement-plan. Аргумент TASK-ID.
---
