...
const STRUCTURAL = {
  ...
  'report-auto-test': [
    ...
    // У раздела про CI ровно два допустимых состояния: сборка (у неё есть
    // номер) либо прямым текстом — почему её не было. Третьего нет: пустой
    // раздел молча читается как «прогнали», а за ним обычно работа, которую
    // никто не сделал.
    {
      test: (t) => {
