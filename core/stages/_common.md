...
- TASK-ID берётся из аргумента. Если не передан — предложи выбор: до 3
  ...
  TASK-ID вручную».
- Тип задачи (FE/BE) и все поля берутся из
...
- в папках артефактов (`docs/specs/tasks/<FE|BE>/<TASK-ID>/` и
  `intents/<INTENT-ID>/`) ты пишешь ТОЛЬКО артефакт СВОЕГО этапа плюс
  `meta.json`. Артефакты соседних
  ...
  `docs/specs/tasks/`, `intents/` и файлы конфигурации в корне (`settings.json`,
