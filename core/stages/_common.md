...
- артефакты задачи (`tasks/<FE|BE>/<TASK-ID>/`) доступны на запись всегда,
  но в папке задачи живут ТОЛЬКО артефакты: `*.md` и `meta.json` — guard
  блокирует там любые другие файлы. ИСХОДНИКИ (tsx/js/less/…) пишутся
  ТОЛЬКО в рабочую копию кодовой базы, НИКОГДА в папку задачи;
