...
- **без активной области плагин в рабочие копии НЕ пишет вовсе.** Поставить
  область — первое действие этапа, а не формальность: пока её нет, guard
  отбивает запись в любой репозиторий из `repos/`. Получил такой отказ —
  значит область не поставлена (или снята раньше времени), а не «guard
  сломался». Правило по типу файла действует всегда: исходник в папке
  артефактов запрещён и с областью, и без неё;
- останавливая этап ДОСРОЧНО (ошибка, грязная копия, эскалация, отказ
  пользователя) — тоже сними область (`scope.mjs clear`); следующий `set` её
  перезаписывает. Устаревшая (старше 8 часов) область продолжает ЗАПРЕЩАТЬ —
