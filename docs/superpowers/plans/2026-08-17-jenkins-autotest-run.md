...
- **Task 0 (discovery MCP) — снята:** имена инструментов не проставляются вовсе. Вместо этого этап ПОДБИРАЕТ их в сессии сам (шаг 7.3 стейджа): три роли — запуск сборки, статус, результаты тестов; несколько кандидатов — вопрос пользователю; инструмента запуска нет — джоба не запускается и это идёт в отчёт. Выбранные инструменты пишутся в meta.json (ciRun.tools).
