...
- **Task 0 (discovery MCP) — снята:** имена инструментов не проставляются вовсе. Вместо этого этап ПОДБИРАЕТ их в сессии сам (шаг 7.3 стейджа): три роли — запуск сборки, статус, результаты тестов; несколько кандидатов — вопрос пользователю; инструмента запуска нет — джоба не запускается и это идёт в отчёт. Подбор идёт каждый прогон заново — в meta.json он не запоминается.
- **В meta.json пишется только `autoTestJob: { url, status }`** рядом с `autotestBranch`. Задуманный в плане объект `ciRun` (джоба, ветка, теги, номер сборки, инструменты) оказался лишним: всё это либо уже есть в meta.json, либо живёт в разделе «Прогон в CI» отчёта.
