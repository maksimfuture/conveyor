...
console.log('Стейдж setup — инициализация и диагностика:');
{
  ...
  // Распознавание 1.x обязано идти ДО диагностики и отменять её. На старом
  // формате ссылки — это ещё `${VAR}`, поэтому repos-status честно отдаёт
  // `outside`/`link-empty` с подсказкой «заполните ссылку»: человек пойдёт
  // править settings.json руками ровно там, где всё чинит миграция.
  // Проверено вживую на репозитории 1.x — четыре строки ложных советов.
  const migStep = algo.indexOf('migrate-workspace');
  const migBeforeDiag = migStep > -1 && diagStep > migStep;
  const skipsDiag = /(не выполняй|НЕ показывай|таблиц[уы] .{0,20}НЕТ|таблицы .{0,30}нет)/i.test(mig);
  if (migBeforeDiag && skipsDiag)
    ok('stage setup: репозиторий 1.x распознаётся до диагностики, и таблица рабочих копий не показывается');
  else
    bad(
      'stage setup: порядок 1.x — ' +
        [
          migBeforeDiag ? null : 'шаг с миграцией стоит не раньше диагностики рабочих копий',
          skipsDiag ? null : 'не сказано, что на 1.x диагностика пропускается и таблица не показывается',
        ]
          .filter(Boolean)
          .join('; '),
    );
...
console.log('Сквозная зачистка удалённых имён:');
{
  ...
  // Разделы, которые ОБЯЗАНЫ называть удалённые имена: README объясняет
  // миграцию, а setup.md по этим именам и распознаёт немигрированный
  // репозиторий — распознавать «ключ, которого нет» иносказательно нельзя.
  const legacySection = {
    'README.md': '## Обновление с 1.x',
    'core/stages/setup.md': '## Признаки версии 1.x',
  };
